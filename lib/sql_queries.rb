...
def selects_the_titles_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) AS amount_left FROM projects JOIN pledges ON pledges.project_id = projects.id GROUP BY projects.title HAVING amount_left <= 0;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount
"SELECT users.name, SUM(pledges.amount) AS total FROM users JOIN pledges WHERE pledges.user_id = users.id GROUP BY users.name ORDER BY total;"
end
